...
from cloc.parsing.extensions._parsing import _parse_complete_buffer, _parse_file

__all__ = ("parse_file_mmap",
           "parse_complete_buffer",
           "parse_buffered_file")

def parse_file_mmap(filepath: str,
    ...
    with open(filepath, 'rb') as file:
        try:
            ...
            return 0, 0
        with mapped_file:
            return _parse_complete_buffer(memoryview(mapped_file),
                                          singleline_symbol,
                                          multiline_start_symbol,
                                          multiline_end_symbol,
                                          minimum_characters)
        
def parse_complete_buffer(filepath: str,
                          singleline_symbol: Optional[bytes] = None,
                          multiline_start_symbol: Optional[bytes] = None,
                          multiline_end_symbol: Optional[bytes] = None,
                          minimum_characters: int = 0) -> tuple[int, int]:
        with open(filepath, 'rb') as file:
            return _parse_complete_buffer(file.read(),
                                          singleline_symbol,
                                          multiline_start_symbol,
                                          multiline_end_symbol,
                                          minimum_characters)
...
def parse_buffered_file(filepath: str,
    ...
    return _parse_file(filepath,
                       singleline_symbol,
                       multiline_start_symbol, multiline_end_symbol,
                       minimum_characters)