...
def test_directory_exclusion_filter():
    ...
    directory_filter: Callable[[str], bool] = construct_directory_filter(
        excluded_directories, exclude=True
    )
...
def test_directory_inclusion_filter():
    ...
    directory_filter: Callable[[str], bool] = construct_directory_filter(
        included_directories, include=True
    )
    for directory in dummy_directories:
        result: bool = directory_filter(directory)
        # Inclusion test
...
def test_extension_inclusion_filter():
    ...
    file_filter: Callable[[str, str], bool] = construct_file_filter(
        included_types, include_type=True
    )
...
def test_extension_exclusion_filter():
    ...
    file_filter: Callable[[str, str], bool] = construct_file_filter(
        excluded_types, exclude_type=True
    )
